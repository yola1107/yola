...
| I45 | P1 | 待验证 | [固定到达率对照](./performance.md#table-worker-固定到达率对照) 中，1,000 桌每桌每秒一次操作、每操作两次四人广播时，8 worker 出现持续积压；16 明显改善等待，32 未稳定更好。Ludo/Whot 已采用固定 `min(tableNum, 16)`，保持桌内串行；完整对局、百人热点房间和客户端延迟尚未验收 | 采集目标负载的 mailbox wait/reject、fanout、Redis/RPC 和客户端 p99；若单桌同步发送仍是瓶颈，先评估定向批量定位和 RPC，定义逐 UID 结果、binding 校验、同玩家顺序与失败反馈时机。现有广播含机器人和离线状态变更，不得直接并发；无进一步证据不增加 outbox 或放大队列 |
