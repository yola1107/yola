...
- <a id="i07"></a> **I07 · P0：历史凭据暴露与生产安全闭环未确认**
  - **影响与证据**：原 I07 记录历史 tracked Redis 口令，须视为已暴露；轮换状态仍为“未确认”。这是历史风险记录，不表示本轮发现了新的明文凭据；不得在文档、日志中复制凭据。
  - **解决方案**：由部署所有者确认受影响凭据范围、轮换和撤销，完成 secret 注入、mTLS/ACL、入口限流配置。访问生产或修改凭据需相应授权，代码审查不能代替部署验收。
  - **验证方案**：在获准环境验证旧凭据失效、最小权限、正确/错误证书、未授权访问与限流；记录去敏的环境和结果，不保存密钥。
  - **关闭条件**：凭据轮换证据与生产安全、限流和容量验收齐备。当前环境无法核验时保留约束，不影响独立的本地框架修复。
