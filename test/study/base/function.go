...
//判断当前时间是否在一个时间之后
func IsCurrTimeAfter(hour, min, sec, nsec int) bool {
	currentTime := time.Now()
	startTime := time.Date(currentTime.Year(), currentTime.Month(), currentTime.Day(),
		hour, min, sec, nsec, currentTime.Location())
...
//两个切片所有元素是否全都一样
func SliceEqual(s1, s2 []int32) bool {
	if len(s1) != len(s2) {
		return false
	}
	for i := 0; i < len(s1); i++ {
...
func GetTimeByStr(d string, t string) (time.Time, bool) {
	...
	if err != nil {
		...
		log.Printf("GetTimeByStr Fail.%+v", err)
		return time.Now(), false
	}
