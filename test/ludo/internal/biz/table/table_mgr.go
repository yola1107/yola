...
// ClientPusher relocates the current Gate binding and pushes a client message.
type ClientPusher interface {
	PushToUID(ctx context.Context, uid string, command int32, msg proto.Message) error
}

// SetClientPusher configures PushToUID-based client delivery for seated players.
...
func newManager(instanceID string, room *conf.Room, repo Repo, workers int, queueSize int, batchSize int) *Manager {
	...
	for index := range manager.tables {
		...
		manager.tables[index] = newTable(tableID, room, repo, tableLog.forTable(tableID), manager)
	}
	return manager
}
...
// Close stops timers and drains accepted table work before closing the table log.
func (m *Manager) Close(ctx context.Context) error {
	if ctx == nil {
		return errors.New("close Ludo table manager: context is nil")
	}
	...
	for _, p := range players {
		...
		for tableIndex < len(m.tables) {
			...
			err := m.call(ctx, target.ID, func(gameTable *Table) error {
				...
				if !gameTable.isFull() && gameTable.robotLogic.canEnter(p) {
					var seatErr error
					joined, seatErr = gameTable.Seat(p)
					return seatErr
				}
				return nil
			})
...
func (m *Manager) restorePlayer(gameTable *Table, p *player.Player) error {
	...
	if err := m.migrationCall(restoreCtx, gameTable.ID, func(current *Table) error {
		var seatErr error
		restored, seatErr = current.Seat(p)
		return seatErr
	}); err != nil {
